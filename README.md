...
- 8 种风格偏好（多选）
- **🆕 小红书风格库** — 顶部今日风格横滚条，16 种内置热门风格（清冷感 / 老钱风 / 多巴胺 / 美拉德 / 纯欲 / 千金风 / 辣妹 / JK / 新中式 / Y2K / 学院风 / 原宿 / 法式慵懒 / 街头机能 / 户外山系 / 极简）一键选中，搭配按该风格重生成
- **🆕 截图导入风格** — 上传一张小红书穿搭截图，视觉模型提取色板 + 关键词，生成专属于你的自定义风格包
- **🆕 真实衣橱 / 参考衣橱 双模式** — 衣橱不足 5 件时自动切到「✨ 参考衣橱」，由通义万相文生图按你选的风格 + 场合 + Dress Code 实时生成 3 套整身搭配大图，依然支持「一键试穿」「收藏」「今日就穿」。收藏页可按「真实搭配 / 灵感」筛选
- AI 多套搭配，**左右滑动切换**，每套附带 LLM 生成的搭配理由与风格角标
...
- **🆕 截图批量导入** — 上传一张淘宝/京东/小红书订单页截图，AI 一次识别多件衣物，勾选后一键加入衣橱（最多 12 件）
- 单品穿搭次数统计
...
| 变量名 | 用途 | 申请地址 | 备注 |
|---|---|---|---|
| `DASHSCOPE_API_KEY` | 搭配理由 + 衣物识别 + **参考衣橱文生图**（首选，便宜） | [阿里云百炼](https://bailian.console.aliyun.com/?apiKey=1#/api-key) | 首次开通有 100 万免费 token；千次搭配理由 ≈ 1 元；参考衣橱文生图 `wanx2.1-t2i-turbo` 约 0.04 元 / 图 |
| `OPENAI_API_KEY` | 一键试穿（图像生成）；搭配理由/识别的备选 | [OpenAI](https://platform.openai.com/api-keys) | 试穿用 `gpt-image-1`，约 0.3 元/次 |

### 配置矩阵（不同 key 组合下的体验）

| `DASHSCOPE_API_KEY` | `OPENAI_API_KEY` | 搭配理由 | 衣物识别 | 一键试穿 | 参考衣橱文生图 |
|:---:|:---:|---|---|---|---|
| ✅ | ✅ | 真 AI（通义） | 真 AI（通义） | 真 AI（OpenAI） | 真 AI（通义万相） |
| ✅ | ❌ | 真 AI（通义） | 真 AI（通义） | Demo 占位图 | 真 AI（通义万相） |
| ❌ | ✅ | 真 AI（OpenAI） | 真 AI（OpenAI） | 真 AI（OpenAI） | Demo 占位图 |
| ❌ | ❌ | 本地模板 | 本地采色 | Demo 占位图 | Demo 占位图 |
...
- [x] **出行装增强**：双模式（智能推荐 / 从必带单品）+ 固定行装收藏
- [x] **截图批量导入 Phase 1**：一张订单页截图 → AI 识别多件 → 一键入橱
